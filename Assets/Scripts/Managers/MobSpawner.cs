...
public class MobSpawner : MonoBehaviour
{
    ...
    void SpawnMobs()
    {
        if (!MobManager.Instance.CanSpawnMobs)
        {
            return;
        }
        ...
        List<Abstract_Mob_Values> Mobs = MobManager.Instance.Mobs.Where(x => x.Difficulty <= StageManager.Instance.GameStage).OrderByDescending(x => x.Difficulty).ToList();
        ...
        foreach (var mob in Mobs)
        {
            ...
            {
                if (MobManager.Instance.CanSpawnMobs && spawnedAmount < MobManager.Instance.SpawnableMobsLeft)
                {
